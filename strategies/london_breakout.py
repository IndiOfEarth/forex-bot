...
class BreakoutSignal:
    ...
    timestamp:        datetime
    vol_scalar:       float = 1.0  # reduced to ATR_HIGH_VOL_SIZE_SCALAR in high-vol regime
...
class LondonBreakout:
    ...
    MAX_RANGE_PIPS   = 80
    REWARD_RISK      = 2.5
    LONDON_OPEN_HOUR = LONDON_OPEN_UTC
    ...
    def _evaluate_pair(
        ...
        # 7b. D1 macro regime filter — EMA 50/200 death/golden cross
        ...
        if ask >= long_entry:
            ...
            if require_daily_trend and d1_trend != "bullish":
                print(f"  ❌ Long breakout blocked — D1 trend not bullish ({d1_trend})")
                return None
            direction   = "buy"
            entry_price = ask
            ...
            if require_daily_trend and d1_trend != "bearish":
                print(f"  ❌ Short breakout blocked — D1 trend not bearish ({d1_trend})")
                return None
            direction   = "sell"
            entry_price = bid
        ...
        # 8b. Momentum body ratio — reject wick breakouts
        #
        ...
        stop_loss, take_profit, stop_pips, target_pips = self._calculate_levels(
            direction, entry_price, asian, pair, atr_regime["current_atr"]
        )
    ...
    def _in_breakout_window(self, now: datetime) -> bool:
        return LONDON_OPEN_UTC <= now.hour < LONDON_OPEN_UTC + 2
